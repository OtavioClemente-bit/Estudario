create function public.get_ai_syllabus_source_metadata(p_path text)
returns table (
  bucket_id text,
  name text,
  owner uuid,
...
begin
  ...
  if p_path is null or p_path !~ ('^' || v_user_id::text || '/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.pdf$') then
...
begin
  ...
  if p_source_object_path is null
     or p_source_object_path !~ ('^' || v_user_id::text || '/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.pdf$') then
...
revoke all on function public.get_ai_syllabus_source_metadata(text) from public;
revoke all on function public.bind_ai_job_source(uuid, text, text, text, bigint, integer, integer, jsonb) from public;
grant execute on function public.get_ai_syllabus_source_metadata(text) to authenticated, service_role;
grant execute on function public.bind_ai_job_source(uuid, text, text, text, bigint, integer, integer, jsonb) to authenticated, service_role;
