...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Escada de reencontro: a questão errada volta sozinha em 3, 10 e 30 dias. */
        val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `error_notebook` ADD COLUMN `retryStreak` INTEGER NOT NULL DEFAULT 0")
        ...
        val MIGRATION_16_17 = object : Migration(16, 17) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `remote_syllabus_sync` ADD COLUMN `attemptToken` TEXT NOT NULL DEFAULT ''")
            }
        }

        val MIGRATION_17_18 = object : Migration(17, 18) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `remote_syllabus_sync` ADD COLUMN `payloadJson` TEXT NOT NULL DEFAULT ''")
            }
        }
        ...
        val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `topics` ADD COLUMN `scopeCovers` TEXT")
                db.execSQL("ALTER TABLE `topics` ADD COLUMN `scopeExcludes` TEXT")
            }
        }

        /** Liga a questão ao conceito de erro que ela testa. */
        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `questions` ADD COLUMN `errorConceptExternalId` TEXT")
            }
        }

        /** Guarda as fontes que a IA declarou ter consultado para gerar cada conteúdo. */
        val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS `content_sources` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                        "`topicId` INTEGER, `packageId` TEXT NOT NULL, `kind` TEXT NOT NULL, `title` TEXT NOT NULL, " +
                        "`publisher` TEXT NOT NULL, `reference` TEXT NOT NULL, `url` TEXT, `accessedAt` TEXT NOT NULL, " +
                        "`externalId` TEXT, `createdAt` INTEGER NOT NULL, " +
                        "FOREIGN KEY(`topicId`) REFERENCES `topics`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE)",
                )
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_content_sources_topicId` ON `content_sources` (`topicId`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_content_sources_packageId` ON `content_sources` (`packageId`)")
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_content_sources_externalId` ON `content_sources` (`externalId`)")
            }
        }

        /** Liga cada questão ao ponto do material que a responde, para a revisão abrir no lugar certo. */
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `questions` ADD COLUMN `reviewAnchor` TEXT")
            }
        }

        /** Guarda no plano o método escolhido no assistente (perfil, bloco, metas). */
        val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `profile` TEXT NOT NULL DEFAULT 'DO_ZERO'")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `blockMinutes` INTEGER NOT NULL DEFAULT 50")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `weeklyQuestionsTarget` INTEGER NOT NULL DEFAULT 100")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `questionsPerTopic` INTEGER NOT NULL DEFAULT 15")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `simulationsPerMonth` INTEGER NOT NULL DEFAULT 2")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `discursivesPerMonth` INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE `study_plans` ADD COLUMN `interleaveSubjects` INTEGER NOT NULL DEFAULT 1")
            }
        }
