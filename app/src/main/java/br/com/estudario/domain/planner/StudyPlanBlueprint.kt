...
object StudyPlanBlueprint {
    ...
    fun build(input: BlueprintInput): BlueprintResult {
        ...
        notes += "Blocos de $block minutos; cada tarefa é um múltiplo disso."
        ...
        val totalWeight = activeSubjects.sumOf { it.weight }.coerceAtLeast(1)
        val theoryPerTopic = StudyMethod.toBlocks(THEORY_BASE_MINUTES, block)
        val questionsPerTopicMinutes = StudyMethod.toBlocks(config.questionsPerTopic * config.minutesPerQuestion, block)
        val streams = mutableListOf<Pair<Int, MutableList<List<TaskDemand>>>>()
        var theoryPlanned = 0
        var questionsPlanned = 0

        activeSubjects.forEach { subject ->
            val subjectTheoryBudget = theoryBudget * subject.weight / totalWeight
            var used = 0
            ...
            if (units.isNotEmpty()) streams += subject.weight to units
        }
        ...
        val weakTopics = input.topics
            ...
            .sortedWith(compareBy({ it.accuracyPercent ?: 100 }, { it.topicId }))
        ...
        if (config.weeklyQuestionsTarget > 0 && plannedQuestions < targetQuestions && heaviestSubject != null) {
            ...
            val batteryMinutes = StudyMethod.toBlocks(perWeek * config.minutesPerQuestion, block)
