...
import br.com.meuconcurso.data.local.QuestionWithOptions
import br.com.meuconcurso.data.local.QuestionSessionEntity
import br.com.meuconcurso.domain.SessionTypeMapper
...
fun QuizScreen(viewModel: AppViewModel, config: QuizConfig, onBack: () -> Unit, onOpenTopic: (Long) -> Unit) {
    ...
    var questionIds by remember(config) { mutableStateOf<List<Long>>(emptyList()) }
    ...
    LaunchedEffect(config, eligible.map { it.question.id }) {
        if (questionIds.isEmpty()) questionIds = when (config.mode) {
    ...
    val selections = remember { mutableStateMapOf<Long, String>() }
    val results = remember { mutableStateMapOf<Long, Boolean>() }
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        current.options.sortedBy { it.position }.forEach { option ->
            item(key = option.id) {
            ...
            item {
                ElevatedCard(colors = CardDefaults.elevatedCardColors(containerColor = if (correct == true) MaterialTheme.colorScheme.secondaryContainer else MaterialTheme.colorScheme.errorContainer)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        if (correct == false) TextButton(onClick = { onOpenTopic(current.question.topicId) }) { Text("Abrir resumo relacionado") }
                    }
                }
            }
        }
