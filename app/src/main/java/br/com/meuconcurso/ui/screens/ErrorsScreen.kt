...
fun ErrorsScreen(viewModel: AppViewModel, onTrainErrors: () -> Unit, onOpenTopic: (Long) -> Unit) {
    ...
    var editing by remember { mutableStateOf<br.com.meuconcurso.data.local.ErrorNotebookEntryEntity?>(null) }
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(onClick = onTrainErrors, enabled = errors.isNotEmpty(), modifier = Modifier.weight(1f)) { Text("Treinar meus erros") }
            }
