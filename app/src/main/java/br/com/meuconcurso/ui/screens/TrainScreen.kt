...
fun TrainScreen(viewModel: AppViewModel, onStart: (QuizConfig) -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(18.dp)) {
        ...
        if (questions.isEmpty()) {
            item { EmptyState("Sem questões", "Importe um pacote .estudo ou carregue os dados demonstrativos em Mais.") }
        } else {
            item {
            ...
            item {
                Button(onClick = { onStart(QuizConfig(count, topicId, subjectId, mode, board, difficulty)) }, Modifier.fillMaxWidth().height(52.dp)) {
