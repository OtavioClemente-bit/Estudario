...
class StudyPlannerEngine(
    ...
    fun plan(snapshot: StudyPlannerSnapshot, reason: ReplanReason): PlanningProposal {
        ...
        fun allocate(demand: TaskDemand, maximum: Int): Int {
            var left = minOf(maximum, remaining.getValue(demand.id))
            var allocated = 0
            while (left > 0) {
                val date = dates.firstOrNull { freeByDate.getValue(it) > 0 } ?: break
                val free = freeByDate.getValue(date)
                val chunk = minOf(left, free, 60.coerceAtLeast(snapshot.policy.minimumBlockMinutes))
        ...
        do {
            ...
            eligible.forEach { demand ->
                ...
                val amount = allocate(demand, minOf(demandRemaining, capLeft, 60))
                if (amount > 0) {
    ...
    private fun demandComparator(snapshot: StudyPlannerSnapshot): Comparator<TaskDemand> {
        ...
        return compareByDescending<TaskDemand> {
