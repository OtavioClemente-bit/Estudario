...
| `persist_ai_job_provider_response` (alter existing) | Existing `(job_id,response_id,lease_owner,lease_token,lease_generation)` | `service_role` | Lock row; matching live lease; processing; compatible `IN_FLIGHT`/quarantine; ID null or equal. | Set ID + `ACCEPTED`, clear creation quarantine. No quota change; lease retained. Same ID is idempotent; different ID conflicts. |
...
- [ ] Ensure both normal persistence and recovery reset the old processing deadline and do not enable normal claim when cancellation is pending.
...
1. Confirm current cloud migration head, deployed function versions, configured secrets, and Cron paused state using read-only checks.
...
4. Deploy the complete `ai-syllabus-worker` bundle, including shared provider code; then deploy complete `ai-syllabus-cancel` and `ai-syllabus-jobs` bundles if changed. Avoid partial shared-module deployment; do not use `--no-verify-jwt` or alter worker auth configuration.
5. If any Edge deploy fails, stop; do not activate Cron. Run remote smoke tests against non-production/test jobs only, covering RPC grants, start/cancel linearization, provider stubs where possible, recovery, and reconciliation lease.
